...
int main()
{
    ...
    do
    {
        printf("\033[1;32m[roditelj]:%s$ \033[0m", getcwd(path, sizeof(path))); 
        ...
        if (fgets(buffer, vel_buf, stdin) != NULL)
        {
            ...
            if (strcmp(argv[0], "cd") == 0)
            {
                ...
                while (argv[argctmp] != NULL)
                {
                    ...
                    if (argv[argctmp + 1] != NULL)
                        strcat(argvtmp, " ");
                    argctmp++;
                }
                if (chdir(argvtmp) != 0)
                    fprintf(stderr, "Neuspjesno otvaranje datoteke.\n");
            ...
            if (strcmp(argv[0], "kill") == 0 && argv[1] && argv[2])
            {
                ...
                {
                    fprintf(stderr, "Neuspjesno gasenje procesa.\n");
                }
                history[pthistory++] = join_argv(argv);
                continue;
            }
            ...
            if (argv[0][0] == '!' && isdigit(argv[0][1]))
            {
                ...
                if (index > 0 && index <= pthistory)
                {
                    ...
                    if (prijenos[0] != NULL)
                    {
                        ...
                        printf("\033[1;33m[dijete %d] krenulo s radom\033[0m\n", pid_novi);
                        procesi[ptprocesi++] = (int)pid_novi;
                        naredbe[ptnaredbe++] = join_argv(prijenos);
                        if (u_pozadini == 1)
                        {
                            prijenos[c] = "&";
                            prijenos[c + 1] = NULL;
                        }
                        history[pthistory++] = join_argv(prijenos);
                        ...
                        if (!u_pozadini)
                        {
                            ...
                            do
                            {
                                ...
                                {
                                    printf("\n\033[1;33m[dijete %d] zavrsilo s radom\033[0m\n", pid_zavrsio); 
                                    tcsetpgrp(STDIN_FILENO, getpgid(0));
                        ...
                        {
                            printf("[roditelj] pokrenuo pozadinski proces %d\n", pid_novi);
                        }
                    }
                    else
                    {
                        printf("Ne vazeca naredba.\n");
                    }
                    free(history_entry);
                }
                else
